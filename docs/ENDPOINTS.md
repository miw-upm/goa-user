...
| GET | `/users/edit-profile/{id}/{token}` | `readByUrlIdWithToken` | `permitAll` |
| PUT | `/users/edit-profile/{id}/{token}` | `updateByUrlIdWithToken` | `permitAll` |
...
Notas:
- `POST /access-links` devuelve, entre otros, `token` en la respuesta de creacion y no se podrá recuperar de otra forma.
